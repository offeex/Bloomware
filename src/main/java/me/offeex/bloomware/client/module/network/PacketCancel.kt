...
object PacketCancel : Module("PacketCancel", "Cancels packets.", Category.NETWORK) {
    ...
    private fun onPacketSend(event: EventPacket.Send) {
        c2s.process(event) { event.canceled = true }
    }

    @Subscribe
    private fun onPacketReceive(event: EventPacket.Receive) = s2c.process(event) { event.canceled = true }
