...
use crate::kocklin_object;
use crate::logging::log_error;

kocklin_object! {
    [me_offeex_bloomware_api_rust] NativeFontRasterizer {
...
impl NativeFontRasterizerTrait for NativeFontRasterizer {
    ...
    fn nativeLoadFont(mut env: JNIEnv, _class: JClass, font: JByteArray, name: JString) -> jint {
        ...
        match font_result {
            ...
            Err(err) => {
                log_error(&format!("Error loading font: {}", err));
                -1
            }
        }
    }
    ...
    fn nativeGetGlyphCount(_env: JNIEnv, _class: JClass, font: jint) -> jint {
        unsafe {
            ...
            if loaded_font.is_none() {
                log_error(&format!("Error getting glyph count: Font with id {} does not exist", font));
                return -1;
            }
    ...
    fn nativeGetName(env: JNIEnv, _class: JClass, font: jint) -> jstring {
        unsafe {
            let loaded_font = FONTS.get(&font);
            if loaded_font.is_none() {
                log_error(&format!("Error getting font name: Font with id {} does not exist", font));
    ...
    fn nativeDeleteFont(_env: JNIEnv, _class: JClass, font: jint) -> jboolean {
        unsafe {
            ...
            if !success {
                log_error(&format!("Error deleting font: Font with id {} does not exist", font));
            }
